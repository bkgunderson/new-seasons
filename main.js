...
let listOfSongObjects = []; // an array of instances of Song
let listOfSectionObjects = []; // an array of instances of Section
let listOfSectionTypeObjects = []; // an array of instances of SectionType
let listOfEventsNSObjects = []; // an array of instances of EventNS
...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    listOfEventsNSObjects.forEach(
      (eve) =>
        (document.getElementById("event-list").innerHTML +=
          '<span id="event-0' +
          listOfEventsNSObjects.indexOf(eve) +
          '-days-since"></span><br>')
    );
    ...
    function updateTime() {
      var d = new Date();
      for (var n in listOfEventsNSObjects) {
        calcDaysSinceEvent(
          d,
          listOfEventsNSObjects[n].dateStart,
          `event-0${n}-days-since`
        );
      }
    }
    function calcDaysSinceEvent(now, then, elem) {
      document.getElementById(elem).innerHTML = `${Math.abs(
        Math.trunc((now - then) / 86400000)
